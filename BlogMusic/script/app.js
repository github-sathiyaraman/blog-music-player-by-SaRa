...
function disSongs() {
    ...
    for (let m = 0; m < list0; m++) {
        //let's show on trending song list
  ...
  <audio src="${l[m].path}" ></audio>
    ...
    var ulTagtrend = document.getElementById("ul-trend");
    ulTagtrend.innerHTML = ""
  ...
  <p class="playlist-card-name">${songs[i].name}</p>
        ...
        ulTagtrend.insertAdjacentHTML("beforeend", liTag1);
    ...
    <p ><span class="name" >${songs[i].name}</span></p> 
                  </div>
                  </li>`;
